...
class ProxyManager {
	...
	public function checkRules($data)
	{
		...
		$nd = Database::querySingleLine("nodes", Array("id" => $data['node']));
		if($nd['need_verified'] == '1') {
		    if($rs['verified'] == '0') {
		        return Array(false, "此节点要求您在创建隧道前完成实名认证");
		    }
		}
		...
		if(!Regex::isNumber($data['local_port']) || Intval($data['local_port']) < 0 || Intval($data['local_port']) > 65535) {
			return Array(false, "本地端口不合法，必须是数字且大于 0 小于 65535。");
		}
		...
		if($data['proxy_type'] !== "http" && $data['proxy_type'] !== "https") {
			...
			if(!Regex::isNumber($data['remote_port']) || Intval($data['remote_port']) < $_config['proxies']['min'] 
				...
				return Array(false, "远程端口不合法，必须是数字且大于 {$_config['proxies']['min']} 小于 {$_config['proxies']['max']}。");
			}
...
EOF;
			...
			if($item[3] == "http" || $item[3] == "https") {
				// HTTP / HTTPS
				$domain = json_decode($item[9], true);
				$configuration .= "custom_domains = {$domain[0]}\n";
				$configuration .= $item[10] == "" ? "" : "locations = {$item[10]}\n";
				$configuration .= $item[11] == "" ? "" : "host_header_rewrite = {$item[11]}\n";
				$configuration .= $item[13] == "" ? "" : "header_X-From-Where = {$item[13]}\n";
			} else {
				// TCP / UDP / XTCP / STCP
				$configuration .= "remote_port = {$item[6]}\n";
				$configuration .= $item[12] == "" ? "" : "sk = {$item[12]}\n";
			}
			
			// 压缩和加密
			$configuration .= $item[7] == "" ? "" : "use_encryption = {$item[7]}\n";
			$configuration .= $item[8] == "" ? "" : "use_compression = {$item[8]}\n";
			$configuration .= "\n";
		}
