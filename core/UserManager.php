...
class UserManager {
	...
	public function doFindpass($data)
	{
		...
		return Array("status" => true, "message" => "验证邮件已发送");
	}
	...
	public function sendFindpassEmail($username, $email, $link)
	{
		...
		$type  = SYTCraftPanel\Utils::isHttps() ? "https" : "http";
	...
	public function updateUser($id, $data)
	{
		...
		if(is_array($rs)) {
			...
			if(isset($data['inbound'], $data['outbound']) && SYTCraftPanel\Regex::isNumber($data['inbound']) && SYTCraftPanel\Regex::isNumber($data['outbound'])) {
