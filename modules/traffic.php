...
if(isset($_GET['getinfo']) && preg_match("/^[0-9]{1,10}$/", $_GET['getinfo'])) {
	...
	if(is_array($rs)) {
		$hs = SYTCraftPanel\Utils::http("http://admin:{$rs['admin_pass']}@{$rs['ip']}:{$rs['admin_port']}/api/serverinfo");
		if(isset($hs['status']) && $hs['status'] == 200) {
			$js = json_decode($hs['body'], true);
			$tf_in  = SYTCraftPanel\Utils::getFormatTraffic($js['total_traffic_in']);
			$tf_out = SYTCraftPanel\Utils::getFormatTraffic($js['total_traffic_out']);
			echo <<<EOF
...
<table style="width: 100%;" class="sinfotable">
	...
	<tr><th>入网流量</th><td>{$tf_in}</td></tr>
	<tr><th>出网流量</th><td>{$tf_out}</td></tr>
...
if(isset($_GET['gettraffic']) && preg_match("/^[0-9]{1,10}$/", $_GET['gettraffic']) && in_array($_GET['type'], ["tcp", "udp", "http", "https", "stcp"])) {
	...
	if(is_array($rs)) {
		$hs = SYTCraftPanel\Utils::http("http://admin:{$rs['admin_pass']}@{$rs['ip']}:{$rs['admin_port']}/api/proxy/{$_GET['type']}");
		if(isset($hs['status']) && $hs['status'] == 200) {
			$js = json_decode($hs['body'], true);
			echo '<table class="table table-bordered table-hover dataTable dtr-inline" style="width: 100%;font-size: 15px;">';
			...
			foreach($js['proxies'] as $proxy) {
				...
				$tf_in  = SYTCraftPanel\Utils::getFormatTraffic($proxy['today_traffic_in']);
				$tf_out = SYTCraftPanel\Utils::getFormatTraffic($proxy['today_traffic_out']);
		...
		<div class="row mb-2">
			<div class="col-sm-6">
				<h1 class="m-0 text-dark">
					<?php echo $page_title; ?>&nbsp;&nbsp;<small class="text-muted text-xs">查看服务器的流量统计信息</small>
				</h1>
			</div>
			<div class="col-sm-6">
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-8">
                    ...
                    <div class="card-body p-0 table-responsive">
						<table class="table table-bordered table-hover dataTable dtr-inline" style="width: 100%;font-size: 15px;">
							<tr>
								...
								<th class='text-center' nowrap>IP地址</th>
							...
							foreach($rs as $node) {
								...
								<td class='text-center' nowrap><a href='javascript:selectserver({$node[0]})'>[选择]</a> | <a href='http://{$node[3]}:7500' target='_blank'>[管理]</a></td>
								";
							}
							?>
						</table>
                ...
                <div id="trafficlist" class="card" style="display: none;">
                    <div class="card-header border-0">
                        <div class="d-flex justify-content-between">
                            <h3 class="card-title">流量排行</h3>
                        </div>
                    </div>
					<div class="card-body p-0 table-responsive">
...
function gettraffic(type) {
    if(nodeid == undefined) {
        Swal.fire({
            title: "发生错误",
            text: "请先选择一个服务器后再查询流量",
            icon: "error"
        });
        return;
    }
    $("#trafficlist").html("正在查询...").show();
    var htmlobj = $.ajax({
        type: 'GET',
