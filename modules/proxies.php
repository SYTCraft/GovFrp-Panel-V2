	...
	if($rs) {
		if(isset($rs['username']) && $rs['username'] == $_SESSION['user']) {
			...
			<table class="proxyinfo" style="width: 100%;font-size: 15px;">
				<tr>
					<h3><?php echo $ns['name']; ?>节点</h3>
				</tr>
				<tr>
					<th>连接地址：</th>
					<td><?php echo "{$ns['hostname']}:{$rs['remote_port']}"; ?></td>
				</tr>
				<tr>
					<th>隧道类型：</th>
					<td><?php echo strtoupper($rs['proxy_type']); ?> 隧道</td>
				</tr>
				<tr>
					<th>本地地址：</th>
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
						...
						<div class="card-body" oncopy="return false" oncut="return false;" onselectstart="return false" oncontextmenu="return false">
							<div class="callout callout-warning">
							    ...
							    <i style="color:#d39e00;font-size:28px;" class="nav-icon fas fas fa-info-circle"></i>
									您已有 <code><?php echo $use_proxies; ?></code> 条隧道，还可以创建 <code><?php echo $max_proxies == -1 ? "无限制" : ($max_proxies - $use_proxies); ?></code> 条隧道
								</p>
							</div>
						</div>
					...
					<div class="card-body" oncopy="return false" oncut="return false;" onselectstart="return false" oncontextmenu="return false">
						<a href="?page=panel&module=addproxy" class="btn btn-primary btn-block lighter">
							<b>创建隧道</b>
						</a>
					</div>
                </div>
...
function getProxyInfo(id) {
	var htmlobj = $.ajax({
		...
		success: function() {
			Swal.fire("隧道信息", htmlobj.responseText);
			return;
		}
	});
}
...
function toggleProxy(id) {
	var htmlobj = $.ajax({
		...
		success: function() {
			Swal.fire("提示信息", htmlobj.responseText, "success");
			return;
		}
	});
}
